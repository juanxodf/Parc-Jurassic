...
use App\Http\Requests\Simulacion\BrechaRequest;
use App\Http\Requests\Simulacion\NormalRequest;
use App\Http\Resources\SimulacionResource;
...
{
    public function normal(NormalRequest $request): JsonResponse
    {
        $validated = $request->validated();
        $celdas = array_key_exists('celda_id', $validated)
            ? Celda::with('dinosaurios')->whereKey($validated['celda_id'])->get()
            : Celda::with('dinosaurios')->get();
        $resultados = [];
        ...
        foreach ($celdas as $celda) {
            ...
            $alertas = [];
            if ($nuevaComida < 20) {
                $alertas[] = 'Alimento crítico';
            }
            if ($nuevasAverias > 3) {
                $alertas[] = 'Muchas averías pendientes';
            }
        ...
        $sim = Simulacion::create([
            ...
            'celda_id' => $validated['celda_id'] ?? null,
    ...
    public function brecha(BrechaRequest $request): JsonResponse
    {
        $validated = $request->validated();
        $celda = $request->filled('celda_id')
            ? Celda::with('dinosaurios')->findOrFail($validated['celda_id'])
    ...
    public function historial(): JsonResponse
    {
        ...
        return response()->json(SimulacionResource::collection($sims), 200);
    }
}
